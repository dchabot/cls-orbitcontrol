...
#include <utils.h> /*usecSpinDelay()*/
#include <psDefs.h>

void UpdateSetPoint(int fd, uint32_t vmeAddr, uint32_t channel, uint32_t setpoint) {
	int dacSetPoint;
    uint32_t value = 0;
    int rc;
...
void ToggleUpdateBit(int fd, uint32_t vmeAddr, uint32_t channel, uint32_t setpoint) {
