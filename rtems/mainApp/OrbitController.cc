...
Ocm* OrbitController::getOcmById(const string& id) {
	...
	if(it != ocmMap.end()) {
		...
		syslog(LOG_INFO, "Failed to find %s in ocmMap!!\n",
				id.c_str());
		return NULL;
	}
}
...
void OrbitController::updateAllOcmSetpoints() {
	...
	for(it=ocmMap.begin(); it != ocmMap.end(); it++) {
		it->second->setSetpoint(it->second->getSetpoint());
	}
}
*/
...
Bpm* OrbitController::getBpm(const string& id) {
	map<string,Bpm*>::iterator it;
	it = bpmMap.find(id);
	if(it != bpmMap.end()) {
		return it->second;
...
rtems_task OrbitController::bpmThreadBody(rtems_task_argument arg) {
	...
	for(;;) {
		...
		numSamplesSummed += sumAdcSamples(sums, rdSegments);
