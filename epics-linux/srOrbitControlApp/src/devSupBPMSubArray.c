...
static long init_record(struct subArrayRecord* sar) {
	...
	printf("sar->inp.value.pv_link.pvname=%s\n",sar->inp.value.pv_link.pvname);
	/*pr = (struct dbCommon *)(sar->inp.value.pv_link.precord);*/
	/*pr = (((struct dbAddr *)((&sar->inp)->value.pv_link.pvt)));*/
	pr = dbGetPdbAddrFromLink(&sar->inp);
	if(pr == NULL) {
		printf("Can't get dbAddr ptr from dbGetPdbAddrFromLink(&sar->inp)!!\n");
		return(-1);
	}
